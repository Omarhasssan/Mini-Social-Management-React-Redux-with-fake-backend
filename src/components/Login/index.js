...
class Login extends Component {
	...
	validate()
	{		
	   this.props.dispatch(login({name:this.state.username,pw:this.state.password}));				
	}
	handleChange(e)
	{
		this.setState({
	...
	render()
	{
		...
		return(
			<div>
				...
				<div className="column">
					<button disabled={username.length == 0 || password.length == 0} onClick={()=>this.validate(this.name,this.pw)}>Login</button>
				</div>
