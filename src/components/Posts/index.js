...
const {userPage,usedFor,posts,users,user,comments} = this.props
	let vr;
	let _this = this;
	if(usedFor == 'UserPage')
	return(
					...
					<div className="p-0 col-auto post-owner post-text">
						{
							...
							<p>
							 {username[0].account.name} shared his post
							</p>
							:
							p.action.shared && username[0].id != p.action.postOwnerId?
							<p>
							 {username[0].account.name} shared {users.find(u=>u.id == p.action.postOwnerId).account.name} post
							</p>
							:<p>{username[0].account.name}</p>
