...
class Comment extends Component{
	...
	render()
	{
		...
		*txt
		*/
		const {comments,ReplyOwner,users,post,user,CommentOwner,comment,commentR,useFor,mentionedReply,userMentionedReply,handleReply,target} = this.props;
		const{expand} = this.state;
		let CmntOwner;
		let _this = this;
		if(useFor == "replies")
			CmntOwner = ReplyOwner
		else
			CmntOwner = CommentOwner;
							...
							<div className="col-auto">
						 		...
						 		<a style={{color:'white'}} onClick={()=>{
						 			if(useFor == "replies")
							 			...
							 			_this.props.dispatch(ReplyForReply(-1,-1,'REMOVE_REPLY'))
